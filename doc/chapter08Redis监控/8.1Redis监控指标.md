...
![image-20240308164244053](https://gitee.com/dongguo4812_admin/image/raw/master/image/202403081642336.png)

在jar包所在目录，使用cmd命令

```shell
java -jar redis-stat-0.4.14.jar 192.168.122.131:6379 --server=63800 --auth=root
```

- 192.168.122.131:6379

​		Redis服务器的IP地址和端口号。`192.168.122.131`是Redis服务器的IP地址，`6379`是Redis服务的端口号。

- --server=63800

  指定redis-stat的端口号，本地启动访问：http://127.0.0.1:63800/

- --auth=root

  Redis服务器密码

  控制台显示Redis服务器信息，以及实时运行情况：

![image-20240308170047427](https://gitee.com/dongguo4812_admin/image/raw/master/image/202403081703504.png)



通过Web界面直接访问 http://127.0.0.1:63800/ 地址，如下图所示：

![image-20240308170158206](https://gitee.com/dongguo4812_admin/image/raw/master/image/202403081703856.png)

页面中的图形是根据Redis服务器的状态动态变化的,还是可以满足一般的监控需求的。

![image-20240308170954422](https://gitee.com/dongguo4812_admin/image/raw/master/image/202403081710841.png)
