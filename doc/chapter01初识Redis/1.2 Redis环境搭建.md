...
- redis-check-aof： 用于检查 Redis AOF (Append Only File) 文件的工具。这个工具用来验证 AOF 文件的格式是否正确，以及是否和当前的 Redis 数据保持一致。
- redis-check-rdb： 用于检查 Redis RDB 文件的工具，它可以帮助你检查 RDB 文件的完整性和一致性。
