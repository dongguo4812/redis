...
我们在代码22行打上断点进行debug，可以看出布Guava隆过滤器在预计插入100W个元素，误判率为0.03时，布隆过滤器内部位数组的大小，也就是用来存储布隆过滤器信息的二进制位数为7298440个；使用的hash函数是MURMUR128_MITZ_64，是Guava布隆过滤器中使用的一个哈希函数。MurmurHash 是一种非加密哈希函数，默认经过5次这样的hash函数的运算
